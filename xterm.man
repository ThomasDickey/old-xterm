...
This entry invokes the \fBsend-signal(suspend)\fP action on systems that
support job control.
.TP 8
...
.TP 8
.B "insert()"
This action is a synonym for \fBinsert-seven-bit()\fP
.TP 8
.B "insert-seven-bit()"
...
.TP 8
.B "set-vt-font(\fId/1/2/3/4/e/s\fP [,\fInormalfont\fP [, \fIboldfont\fP]])"
...
used when
\fIxterm\fP was started), \fI1\fP through \fI4\fP indicate the fonts 
specified by the \fIfont1\fP through \fIfont4\fP resources, \fIe\fP or \fIE\fP
...
\fIi\fP or \fII\fP indicate the font selection (as made by programs such as
\fIxfontsel(1)\fP) indicated by the second action argument.
.TP 8
.B "start-extend()"
This action is similar to \fBselect-start\fB except that the 
...
This action sends the signal named by \fIsigname\fP (which may also be a 
number) to the \fIxterm\fP subprocess (the shell or program specified with
...
not significant):
\fIsuspend\fP, \fItstp\fP (if supported by the operating system), \fIcont\fP
...
The old version, along with a first stab at an update, are available in
the sources.
.PP
